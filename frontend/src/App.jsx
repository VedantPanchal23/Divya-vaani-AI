...
// Extracted Header Component (eliminates duplication)
function Header({ theme, toggleTheme, isThemeTransitioning, onLogoClick, onAdminClick, showAdmin }) {
    return (
        <header className="header">
            <div className="header-logo" onClick={onLogoClick} style={{ cursor: 'pointer' }}>
                ...
                <span className="header-logo-text">दिव्य वाणी</span>
                <span className="header-logo-subtitle">Divya Vaani AI</span>
            </div>
            <nav className="header-nav">
                <button
                    className={`btn btn-ghost header-admin-btn ${showAdmin ? 'active' : ''}`}
...
function App() {
    ...
    const filteredVideos = videos.filter(video => {
        const matchesSearch = searchQuery === '' || 
            ...
            (video.tags && (Array.isArray(video.tags) 
                ...
                : typeof video.tags === 'string' && video.tags.toLowerCase().includes(searchQuery.toLowerCase())
            ));
    ...
    const handleBack = () => {
        ...
        setShowAdmin(false);
        fetchVideos(); // Refresh in case new videos were added
    };
    ...
    return (
        <div className={`app ${isThemeTransitioning ? 'theme-transitioning' : ''}`}>
            <Header
                ...
                onLogoClick={() => { setSelectedVideo(null); setShowAdmin(false); fetchVideos(); }}
            ...
            {showAdmin ? (
                ...
                <>
                    ...
                    <section className="welcome-bar">
                        <h1 className="welcome-title">Explore <span>Spiritual Discourses</span></h1>
                        <p className="welcome-sub">
                            Dive into Maharaj Ji's teachings — read summaries, explanations, and ask questions powered by AI.
                        </p>
                        <p className="welcome-sub-hi">महाराज जी के प्रवचनों से दिव्य ज्ञान का अनुभव करें।</p>
                    </section>
                    ...
                    <footer className="footer">
                        <p>🙏 Powered by AI for spiritual learning • दिव्य वाणी</p>
                    </footer>
                </>
            )}
        </div>
